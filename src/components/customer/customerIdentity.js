...
class CustomerIdentity extends React.PureComponent {
  ...
  getIdentitySymbol() {
    ...
    if (
      _.find(indetifyCards, indetify => indetify.identifyDocumentType.typeCode === 'IDENTIFY_CARD') && 
      _.find(indetifyCards, indetify => indetify.identifyDocumentType.typeCode === 'PASSPORT_CARD')
    ) {
      return false;
    }
    return true;
  ...
  addIdentity() {
    ...
    if (!_.find(indetifyCards, indetify => indetify.identifyDocumentType.typeCode === 'IDENTIFY_CARD')) {
      fields.insert(fields.length, defaultIdentityCard);
      return;
    }
    if (!_.find(indetifyCards, indetify => indetify.identifyDocumentType.typeCode === 'PASSPORT_CARD')) {
      fields.insert(fields.length, defaultPassportCard);
      return;
    }
  }
