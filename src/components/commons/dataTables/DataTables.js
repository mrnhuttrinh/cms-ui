...
class DataTables extends Component {
  ...
  render() {
    ...
    if (showFooterToolbar) {
      const totalPages = Math.ceil(count / rowSize);
      ...
      if (showEllipsis) {
        footerToolbar = (
          <Pagination
            ...
            boundaryPagesRange
            siblingPagesRange
            onChange={this.props.onPageChangeFromPagination}
          />
        );
      } else {
        footerToolbar = (
    ...
    const startItem = (page - 1) * rowSize;
    const lastItem = page * rowSize;
    const dataRender = _.slice(data, startItem, lastItem);
    return (
      <div>
        {headerToolbar}
        <DataTablesTable
